<%-- 
    Document   : offloadingadd
    Created on : Jun 24, 2015, 9:37:04 PM
    Author     : gwawan
--%>
...
public String drawList(Vector objectClass) {
    ...
    jsplist.addHeader("Latin Name", "32%");
    ...
    for (int i = 0; i < objectClass.size(); i++) {
        ...
        rowx.add(offLoading.getLatinName());
...
if(deepSlope.getLandingDate() != null) listOfBoat = com.project.insite.DbOffLoading.listOfBoat(deepSlope.getPartnerId(), JSPFormater.formatDate(landingDate, "yyyy-MM-dd"));

Vector listOffloading = new Vector();
String where = com.project.insite.DbOffLoading.colNames[com.project.insite.DbOffLoading.COL_BOAT] + " ilike '%" + boatName + "%' and "
        + " date_trunc('day', " + com.project.insite.DbOffLoading.colNames[com.project.insite.DbOffLoading.COL_RECEIVED_DATE] + ") = '" + JSPFormater.formatDate(landingDate, "yyyy-MM-dd") + "'"
        + " and " + com.project.insite.DbOffLoading.colNames[com.project.insite.DbOffLoading.COL_PARTNER_ID] + " = " + deepSlope.getPartnerId()
        + " and " + com.project.insite.DbOffLoading.colNames[com.project.insite.DbOffLoading.COL_STATUS] + " = 0";
if(boatName.length() > 0 && (iJSPCommand == JSPCommand.LOAD || iJSPCommand == JSPCommand.SAVE)) {
    listOffloading = com.project.insite.DbOffLoading.list(0, 0, where, "");
}

//Convert data from Insite into I-Fish Community data structure
if(iJSPCommand == JSPCommand.SAVE && listOffloading != null && listOffloading.size() > 0) {
    for(int i = 0; i < listOffloading.size(); i++) {
        com.project.insite.OffLoading offLoadingInsite = (com.project.insite.OffLoading) listOffloading.get(i);
        ...
        if(fishId != 0) {
            ...
            try {
                long oid = DbOffLoading.insertExcWithOid(offLoading);
                DbLogsAction.insertLogs(JSPCommand.ADD, DbOffLoading.DB_OFFLOADING, oid, user.getOID());

                //Update Insite data status
                if (oid != 0) {
                    offLoadingInsite.setStatus(1);
                    com.project.insite.DbOffLoading.updateExc(offLoadingInsite);
                }
            } catch (Exception e) {
                System.out.println(e.toString());
            }
...
function cmdSearch() {
    document.frmoffloadingadd.command.value="<%=JSPCommand.LOAD%>";
    document.frmoffloadingadd.action="smoffloadingadd.jsp";
    document.frmoffloadingadd.submit();
}

<%if (privAdd || privEdit) {%>
function cmdSave() {
    document.frmoffloadingadd.command.value="<%=JSPCommand.SAVE%>";
    document.frmoffloadingadd.action="smoffloadingadd.jsp";
    document.frmoffloadingadd.submit();
}
<%}%>
...
<body>
    ...
    <form name="frmoffloadingadd" method="post" action="">
                    ...
                    <table width="100%" border="0" cellspacing="2" cellpadding="0">
                        ...
                        <tr align="left" valign="top">
                            ...
                            <td><input name="landing_date" value="<%=JSPFormater.formatDate((landingDate == null) ? deepSlope.getLandingDate() : landingDate, "dd/MM/yyyy")%>" size="11" style="text-align:center" readOnly>
                                <a href="javascript:void(0)" onClick="if(self.gfPop)gfPop.fPopCalendar(document.frmoffloadingadd.landing_date);return false;" ><img class="PopcalTrigger" align="absmiddle" src="<%=rootSystem%>/calendar/calbtn.gif" height="19" border="0" alt="Landing Date"></a>
                            </td>
                        </tr>
                        <tr align="left" valign="top">
                            <td>Boat/Supplier&nbsp;</td>
                        ...
                        <tr align="left" valign="top">
                            <td colspan="2"><b>List of off-loading data from partner database</b></td>
                        </tr>
                        <% if(iJSPCommand == JSPCommand.LOAD) { %>
                        <tr align="left" valign="top">
                            <td colspan="2"><%=listOffloading.size()>0?drawList(listOffloading):"Data not found"%>&nbsp;</td>
                        </tr>
                        <% } %>
                    </table>
                </td>
