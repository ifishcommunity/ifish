<%-- 
    Document   : partner
    Created on : Apr 21, 2015, 10:01:09 PM
    Author     : gwawan
--%>
<%@ page language="java" %>
...
public String drawList(Vector objectClass) {
    ...
    for (int i = 0; i < objectClass.size(); i++) {
        Partner partner = (Partner) objectClass.get(i);
        Vector rowx = new Vector();

        rowx.add(partner.getName());
        rowx.add(partner.getType());
        rowx.add(partner.getAddress());
        rowx.add(partner.getCp());
        rowx.add(partner.getPhone());
        rowx.add(partner.getNotes());

        lstData.add(rowx);
        lstLinkData.add(String.valueOf(partner.getOID()));
    }
...
int recordToGet = 15;
String where = "";
String order = DbPartner.colNames[DbPartner.COL_TYPE];//+", "+DbPartner.colNames[DbPartner.COL_CODE];
Vector listPartner = new Vector(1, 1);
JSPLine jspLine = new JSPLine();

/* GET REQUEST FROM HIDDEN TEXT */
int listJSPCommand = JSPRequestValue.requestInt(request, "list_command");
int start = JSPRequestValue.requestInt(request, "start");
String srcType = JSPRequestValue.requestString(request, "src_type");
String srcName = JSPRequestValue.requestString(request, "src_name");

if(srcType.length() > 0) {
    where = DbPartner.colNames[DbPartner.COL_TYPE] + " like '" + srcType + "'";
}
...
if(srcName.length() > 0) {
    ...
    where += DbPartner.colNames[DbPartner.COL_NAME] + " ilike '%" + srcName + "%'";
}
...
Vector valType = new Vector();
valType.add("all");
valType.add(IFishConfig.PARTNER_FISHERMAN);
valType.add(IFishConfig.PARTNER_FISH_TRADER);
valType.add(IFishConfig.PARTNER_COMPANY);
Vector keyType = new Vector();
keyType.add("");
keyType.add(IFishConfig.PARTNER_FISHERMAN);
keyType.add(IFishConfig.PARTNER_FISH_TRADER);
keyType.add(IFishConfig.PARTNER_COMPANY);

/* control list */
CmdPartner cmdPartner = new CmdPartner(request);
int vectSize = DbPartner.getCount(where);
start = cmdPartner.actionList(listJSPCommand, start, vectSize, recordToGet);
listPartner = DbPartner.list(start, recordToGet, where, order);
/* end */
%>
<!-- End of JSP Block -->
...
<script language="JavaScript">
    ...
    function cmdList(){
        document.frmpartner.start.value="0";
        document.frmpartner.list_command.value="<%=JSPCommand.LIST%>";
        document.frmpartner.action="partner.jsp";
        document.frmpartner.submit();
    }

    function cmdListFirst(){
        document.frmpartner.list_command.value="<%=JSPCommand.FIRST%>";
        document.frmpartner.action="partner.jsp";
        document.frmpartner.submit();
    }

    function cmdListPrev(){
        document.frmpartner.list_command.value="<%=JSPCommand.PREV%>";
        document.frmpartner.action="partner.jsp";
        document.frmpartner.submit();
    }

    function cmdListNext(){
        document.frmpartner.list_command.value="<%=JSPCommand.NEXT%>";
        document.frmpartner.action="partner.jsp";
        document.frmpartner.submit();
    }

    function cmdListLast(){
        document.frmpartner.list_command.value="<%=JSPCommand.LAST%>";
        document.frmpartner.action="partner.jsp";
        document.frmpartner.submit();
    }
<!--
                  ...
                  <table width="100%" border="0" cellspacing="0" cellpadding="0">
                    <tr>
                      <td><!-- #BeginEditable "content" -->
                          ...
                          <input type="hidden" name="partner_oid" value="">
                          <table width="100%" border="0" cellspacing="0" cellpadding="0">
                            <tr>
                              <td class="title">
                            ...
                            <tr>
                                <td class="container">
                                    <table width="100%" border="0" cellspacing="2" cellpadding="0">
                                        ...
                                        <tr align="left" valign="top">
                                            <td width="10%">Type&nbsp;</td>
                                            <td width="30%"><%=JSPCombo.draw("src_type", null, srcType, keyType, valType, "") %>&nbsp;</td>
                                            <td width="60%">&nbsp;</td>
                                        </tr>
                                        <tr align="left" valign="top">
                                            <td width="10%">Name&nbsp;</td>
                                        ...
                                        <tr>
                                            <td colspan="3"><%=drawList(listPartner)%></td>
                                        </tr>
                                        <%} else {%>
                                        <tr>
                                            <td colspan="3">
