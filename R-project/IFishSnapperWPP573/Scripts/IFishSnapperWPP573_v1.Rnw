...
<<globaloptions,echo=FALSE>>=
  opts_chunk$set(echo=FALSE, fig.path='/root/R-project/IFishSnapperWPP573/Plots/plot-', cache=FALSE)
@

\newgeometry{left=1cm,top=1cm,right=1cm,bottom=1cm}
\input{/root/R-project/IFishSnapperWPP573/Texs/coverpage.tex}
\restoregeometry

\input{/root/R-project/IFishSnapperWPP573/Texs/innercoverpage.tex}
\clearpage
...
\large
\input{/root/R-project/IFishSnapperWPP573/Texs/1_introduction.tex}

<<TableSPP, cache=FALSE, fig.width=10, out.width='0.9\\linewidth', echo = FALSE, results = 'asis'>>=
dateY1start <- as.Date("2014-10-05")
dateY1end <- as.Date("2015-10-06")

dateY2start <- as.Date("2015-10-05")
dateY2end <- as.Date("2016-10-06")
...
for(i in sort((unique(SPP$fish_id)))){
    ...
    # Data for Year I, Smart Measuring
    dfx <- subset(df,fish_id==i)
    dfx$landing_date2<-as.Date(substr(as.character(dfx$landing_date),1,10))
    dfSM <- dfx[(dfx$landing_date2 >= dateY1start & dfx$landing_date2 < dateY1end & dfx$approach == 0),]
    # End data Year I

    # Data for Year II, CODRS
    dfy <- subset(df, fish_id==i)
    dfy$landing_date2<-as.Date(substr(as.character(dfy$landing_date),1,10))
    dfCODRS <- dfy[(dfy$landing_date2 >= dateY2start & dfy$landing_date2 < dateY2end & dfx$approach == 2),]
    # End data Year II

    # Add column for total row data of Year I and Year II
    spx$y1 <- nrow(dfSM)
    spx$y2 <- nrow(dfCODRS)
...
spp2x <- spp2[,c("fish_id", "fish_binary", "reported_trade_limit_weight", "var_a", "var_b", "length_basis", "converted_trade_limit_l", "plotted_trade_limit_tl", "y1", "y2")]
colnames(spp2x) <- c("", "", "Reported", "", "", "Length", "Converted", "Plotted", "", "")
print(xtable(spp2x, format="latex", align="lcccccccccc", digits=c(0,0,0,0,3,3,0,2,2,0,0), caption = 'Length - Weight Relationships \\& Trading Limits Deep Snapper Fisheries Indonesia'), caption.placement = 'top', 
  size="\\small", 
  include.rownames=FALSE,
  add.to.row = list(
    pos = list(0),
    command = c(
      "{ } & { } & {Trade} & \\multicolumn{2}{c}{ W = a L\\textsuperscript{b}} & {Type} & {Trade} & {Trade} & \\multicolumn{2}{c}{Sample Size}\\\\
      { } & { } & {Limit} & { } & { } & {for a \\& b} & {Limit} & {Limit} & { } & { }\\\\
      {\\#ID} & {Species} & {Weight (g)} & {a} & {b} & {TL-FL-SL} & {L(cm)} & {TL(cm)} & {Year I} & {Year II} \\\\"
    )
  )
)
@
...
\input{/root/R-project/IFishSnapperWPP573/Texs/2_materials_and_methods.tex}

\newpage

\chapter{Fishing grounds and traceability}
\input{/root/R-project/IFishSnapperWPP573/Texs/3_fishing_grounds.tex}
...
for(i in sort((unique(SPP$fish_id)))){
    ...
    # This condition will insert new dummies row for dfSM
#    if(nrow(dfSM) == 0) {
#        dfSM <- rbind(dfSM, head(dfCODRS,1))
#        dfSM$cm <- as.numeric(1)
#        dfSM$EcoSize <- as.character("Immature")
#        isDummiesDataY1 <- 1
#    }

    # This condition will insert new dummies row for dfCODRS
#    if(nrow(dfCODRS) == 0) {
#        dfCODRS<- rbind(dfCODRS, head(dfSM,1))
#        dfCODRS$cm <- as.numeric(1)
#        dfCODRS$EcoSize <- as.character("Immature")
#        isDummiesDataY2 <- 1
#    }
...
highestx1 <- 0
if(nrow(dfSM) > 0) {
    lowestx1 <- min(0.5*spx$lmat,min(dfSM$cm))
    highestx1 <- 1.2*spx$lmax
}

    lowestx2 <- min(0.5*spx$lmat,min(dfCODRS$cm))
    highestx2 <- 1.2*spx$lmax

    lowestx = 0
    if(lowestx1 < lowestx2) {
        lowestx = lowestx1
    } else {
        lowestx = lowestx2
    }
    ...
    highestx = 0
    if(highestx1 > highestx2) {
        highestx = highestx1
    } else {
        highestx = highestx2
    }

    ### START YEAR I, SmartMeasuring ###
    gt <- paste("(ID #",spx$fish_id,") ",spx$fish_binary," (",spx$fish_family, "), Year I, n = ", nrow(dfSM),sep="")
    Ltitle <- paste("Year I, (ID #",spx$fish_id,") Length frequency of ",spx$fish_binary," (",spx$fish_family, "), n = ", nrow(dfSM),sep="")

if(nrow(dfSM) == 0) {
    plot(1, type="n", main=Ltitle, xlab="Total Length (cm)", ylab="Frequency", xlim=c(lowestx,highestx), ylim=c(0, 10), bty="n")
} else {
    tmp1 <- hist(dfSM$cm,main=Ltitle,col="lightgrey",xlab="Total Length (cm)", xlim=c(lowestx,highestx), breaks=seq(0, max(dfSM$cm)+10, by=5))
    ...
    perc_imm <- 100*length(dfSM$EcoSize[dfSM$EcoSize=="Immature" & !is.na(dfSM$EcoSize)])/length(dfSM$cm)
    perc_smat <- 100*length(dfSM$EcoSize[dfSM$EcoSize=="SmallMature" & !is.na(dfSM$EcoSize)])/length(dfSM$cm)
    perc_lmat <- 100*length(dfSM$EcoSize[dfSM$EcoSize=="LargeMature" | dfSM$EcoSize=="MegaSpawner" & !is.na(dfSM$EcoSize)])/length(dfSM$cm)
    perc_megasp <- 100*length(dfSM$EcoSize[dfSM$EcoSize=="MegaSpawner" & !is.na(dfSM$EcoSize)])/length(dfSM$cm)
    ...
    # Add legend
    legend("topright", bty="o", c(paste("Trade Limit",formatC(spx$plotted_trade_limit_tl,digits=0,format="f"),"cm",sep=" "), paste("L-maturity",spx$lmat,"cm",sep=" "), paste("L-optimum",spx$lopt,"cm",sep=" "), paste("L-infinity",spx$linf,"cm",sep=" "), paste("L-max",spx$lmax,"cm",sep=" ")), text.col = c(2, 1, 1, 4, 1))
}
###################

    ### START YEAR II, CODRS ###
    gt <- paste("(ID #",spx$fish_id,") ",spx$fish_binary," (",spx$fish_family, "), Year II, n = ", nrow(dfCODRS),sep="")
    Ltitle <- paste("Year II, (ID #",spx$fish_id,") Length frequency of ",spx$fish_binary," (",spx$fish_family, "), n = ", nrow(dfCODRS),sep="")
...
#            "Fmax" = TRUE)
    ...
    #### SPR END HERE
    # Add legend
    legend("topright", bty="o", c(paste("Trade Limit",formatC(spx$plotted_trade_limit_tl,digits=0,format="f"),"cm",sep=" "), paste("L-maturity",spx$lmat,"cm",sep=" "), paste("L-optimum",spx$lopt,"cm",sep=" "), paste("L-infinity",spx$linf,"cm",sep=" "), paste("L-max",spx$lmax,"cm",sep=" "), paste("SPR:", text_SPR, "%", sep=" ")), text.col = c(2, 1, 1, 4, 1, 1))
...
\input{/root/R-project/IFishSnapperWPP573/Texs/5_discussion_and_conclusions.tex}

%\newpage

%\chapter{Next steps}
%\input{/root/R-project/IFishSnapperWPP573/Texs/6_nextsteps.tex}

\newpage

\chapter{References}
\input{/root/R-project/IFishSnapperWPP573/Texs/6_references.tex}
