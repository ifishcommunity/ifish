...
rs <- dbSendQuery(con1, "select f.fish_id, lmat, lmatm, lopt, linf, lmax, landing_date, cm, b.name as boat, approach from ifish_fish f inner join ifish_sizing s on f.oid = s.fish_id inner join ifish_deepslope d on s.landing_id = d.oid inner join ifish_boat b on d.boat_id = b.oid where f.fish_id > 0 and f.fish_family = any(values('Epinephelidae')) and s.data_quality = 1 and ((d.approach = 0 and d.landing_date < '2015-10-06') or (d.approach = 2 and d.landing_date >= '2015-10-06')) and fishery_type = any(values('Snapper')) and doc_status = any(values('Posted'));")
...
knit('/root/R-project/IFishGrouper/Scripts/IFishGrouper.Rnw',output='/root/R-project/IFishGrouper/Scripts/IFishGrouper.tex')
